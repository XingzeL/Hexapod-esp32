...
namespace hexapod_test {
    ...
    namespace {
        ...
        int hexapod2pwm(int legIndex, int partIndex) { 
            //这里有转换到pwm的index，可能对用PCA的引脚
            //用在构造函数上，有legIndex和partIndex时转换成属性pwmIndex
            switch(legIndex) {
    ...
    void Servo::setAngle(float angle) {
        ...
        if (angle > range_ + adjust_angle_) { //adjust angle是软区间
        ...
        pwm->setPWM(idx, 0, us/kTickUs);  //属性pwmIndex_用在这里面，内部调用驱动api，idx决定哪个PCA引脚
