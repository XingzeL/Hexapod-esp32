...
namespace hexapod { 
    ...
    namespace {
        ...
        auto pwmLeft = hal::PCA9685(0x41);  //左边的芯片地址设置为0x41，焊锡连接A0引脚
        auto pwmRight = hal::PCA9685(0x40); //右边的芯片默认
        bool pwmInited = false;
 
        void initPWM(void) {  //启动PCA9685芯片并设置输出频率
            if (pwmInited)
                return;
        ...
        int hexapod2pwm(int legIndex, int partIndex) { //在外面赋值给pwmIndex_
            switch(legIndex) {
    ...
    void Servo::setAngle(float angle) {
        ...
        angle_ = angle;  //Servo的angle_赋值为输入的参数angle

        hal::PCA9685* pwm; //pwm是一个PCA9685对象，可以setPWM
        int idx;
        if (pwmIndex_ < 16) {
            pwm = &pwmRight;
            idx = pwmIndex_;
